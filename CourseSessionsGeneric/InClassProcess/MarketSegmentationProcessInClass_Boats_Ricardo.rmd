...
This questions seem to be contradictory but to aggregate them makes sense as the second one has a negative weight allocated to it. We were expecting this as people who rely other´s opinion wouldn't have to knowledge the repair the boat by themselves, hence they are beginners in boat expertise.  
...
# Please use numbers, not column names, e.g. c(1:5, 7, 8) uses columns 1,2,3,4,5,7,8
segmentation_attributes_used = c(22,10,28,5,9,12,2)  #c(10,19,5,12,3) 
...
The numbers in the above matrix are the averages of the profiling variables (each of 81 questions from the survey) of the different segments identified (4 segments or clusters) relative to the average of the variables across the whole population minus 1. For example, a value of 0.26 means that the given cluster/segment scores at 1.26 x average of whole population for the given question (questions are rows).

*Question 2*

*Status seekers & enthusiasts*

* Focused with their status and want to have latest and greatest boat
* Younger than average of population, like watersports
* Enthusiastic and passionate about boats, that spend a lot of time boating
* People that like their boat, but will buy a different brand next time
* High demands in terms of top customer service and latest gadgets

*Beginner*

* They rely on experts when buying a boat
* They have less boats than average population

*Expert*

* They rely least on the experts when buying a new boat
* They don't spend all their time on the boat

*Older, a bit expert, anti status guy*

* Status least of the concern
* They don't need speed, big boat, brand, aren't perceived as experts
...
Based on our analysis, we identified 4 key segments in the market (the ones described above). 
In order to select the most reasonable number of segments, we decided to run different combinations, taking into account:

* **The number of factors:** we tested different combination of factors/components, from 5 components (eigenvalues above 1) to 12 components (explained 70% of total variance)
* **The number of attributes considered:** only the top attribute of the different factors/components that we found vs. top two attributes
* **Clustering method:** we compared the results from hierarchical vs. k-means clustering

By comparing the different outcomes, we were able to identify **6 key segments** that were always present in the different segmentation analysis (**"status seekers & enthusiasts", "Beginner", "Expert", "Older, a bit expert, anti status guy"**), which gave us confidence about their statistic relevance. On top of this, these segments seem to have also some clear characteristics that the marketing team could target and influence with specific initiatives to boost sales through brand awareness and pricing.
